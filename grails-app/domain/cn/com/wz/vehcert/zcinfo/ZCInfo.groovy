 ...
 * @update  2018-02-08 QJ SAP_status 增加状态2
 */
class ZCInfo {
    String id
    ...
    String veh_Clxh    //车辆型号 也就是公告号与产品公告号时同一个概念
    ...
    String web_status='0' //合格证状态 是否上传    0:未上传 1：上传成功 2：上传失败 ；3已撤销（仅车间使用）
    String web_isprint='0' //合格证是否打印
    String web_virtualcode//虚拟合格证编号
    String vercode//校验信息

    String processType='0' //生成类型 0正常生成；1非常规生成（如试制车、去年车辆合格证更换等）此字段仅在车间程序中使用

    String needUpload ='0' //打印成pdf的合格证，需要经销商点击上传后，在公告资源部的上传页面显示。0表示经销商未上传，1表示经销商已上传

    String SAP_No      // SAP中的车辆唯一号
    String SAP_status='0'  //根据调用SAP返回的结果，显示传送状态：0未同步、1已同步，同步失败也是未同步 2,表示需要传SAP
    ...
    static constraints = {
        ...
        needUpload       (blank: true,nullable: true)
    ...
    static mapping = {
        ...
        web_status    (comment:'合格证状态 是否上传 0 未上传 1 上传成功 2 上传失败 3 需要修改或撤销的合格证  6禁止上传')
        ...
        processType(comment:'车辆生成类型 0正常生成；1非常规生成（如试制车、去年车辆合格证更换等）此字段仅在车间程序中使用')
        needUpload (comment:'打印成pdf的合格证，需要经销商点击上传后，在公告资源部的上传页面显示。0表示经销商未上传，1表示经销商已上传 ')

        SAP_No     (comment:'SAP中的车辆唯一号 ')
        SAP_status (comment:'根据调用SAP返回的结果，显示传送状态：0未同步、1已同步，同步失败也是未同步 ')
        transDate (comment:'传输到SAP的时间 ')

        sort "veh_Zchgzbh": 'desc'
    }
