		...
		<div id="page" style="width:100%;background-color:white;">
	    	<div id="center-panel" style="margin-left:4px;border:1px;visibility:hidden;">
				...
				<div style="margin-right:8px;margin-top:8px;">
					<input type="hidden" id="articleCategoryId">
	 	...
	 	<script>
			...
			$(function() {
                ...
                $('#btn_create').click(function(){
                	   var url = '${createLink(controller:'articleCategory',action:'create')}';
                ...
                $('#btn_edit').click(function(){
             		   var url = '${createLink(controller:'articleCategory',action:'edit')}';
             		   var selected = $('#west-panel').omTree('getSelected');
             		   if(selected==null){
	                ...
	                //这里利用Ajax删除数据
                   	$.omMessageBox.confirm({
                        ...
                        onClose:function(v){
                   		...
                   		//需要判断父节点为空的情况
                   		var parent = $('#west-panel').omTree('getParent',selected);
                   		var deleteUrl = '${createLink(controller:'articleCategory',action:'jsonDelete')}/'+selected.id;
			...
			$(function() {
				...
				//构建控件
			    $('#west-panel').omTree({
			         dataSource : "${createLink(controller:'articleCategory',action:'jsonAllListTree')}", // 后台取数的URL
			       	 simpleDataModel:true,
			       	 onSelect: function(node,event) {
			       		  $('#center-panel').css({visibility:'visible'});
			       		  $('#articleCategoryId').val(node.id);
		                  $('#content').load("${createLink(controller:'articleCategory',action:'jsonShow')}/"+node.id);
