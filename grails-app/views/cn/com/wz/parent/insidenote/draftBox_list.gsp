...
<html>
	<head>
		...
		<g:set var="entityName" value="${message(code: 'draftBox.label', default: 'DraftBox')}" />
...
<%--					<input id="btn_edit" type="button"  class="edit" value="${message(code: 'default.button.edit.label', default: 'Edit')}"/>--%>
					<input id="send" type="button"  class="import" value="${message(code: 'default.button.send.label', default: 'send')}"/>
	 	...
	 	$(function() {
			 ...
			 //绑定查询按钮事件
               $('#btn_query').omButton({
                   		width:80,
                	   onClick:function(){
		   	                    var url = "${createLink(controller:'InsideNote',action:'jsonDraftBoxList')}";
               ...
               $('#btn_edit').click(function(){
            		   var url = '${createLink(controller:'InsideNote',action:'edit')}';
            		   var selected = $('#insideNote_grid').omGrid('getSelections',true);
              ...
              $("#send").click(function(){
              		...
              		$.omMessageBox.confirm({
                        ...
                        onClose:function(v){
                            ...
                            if(v){
                                var url = '${createLink(controller:'InsideNote',action:'jsonSend')}';
              	         	    var sendIds = '';
              	            	$.each(selections,function(i,item){
                   		           sendIds += item.id+'_';
                            	});
    		                   	
    		                   	$.post(url,{"sendIds":sendIds},function(data,textStatus){
               ...
               $('#btn_delete').click(function(){
                   		...
                   		$.omMessageBox.confirm({
                            ...
                            onClose:function(v){
                                if(v){
                                	 var deleteUrl = '${createLink(controller:'InsideNote',action:'jsonDraftBoxDelete')}';
        	                   		var deleteIds = '';
        	                   		$.each(selections,function(i,item){
        		                   		deleteIds += item.id+'_';
        		                   	});
		...
		function buildRightGrid(value,event){
			$('#insideNote_grid').omGrid({
				dataSource:'${createLink(controller:'InsideNote',action:'jsonDraftBoxList')}',
				title:"<g:message code="default.list.label" args="[entityName]"/>",
				singleSelect:false,
				limit : 12, 
				height : 440,
