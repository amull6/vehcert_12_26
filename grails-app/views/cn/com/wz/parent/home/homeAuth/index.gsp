...
<html>
	<head>
		...
		<g:set var="entityName" value="${message(code: 'role.label', default: 'Role')}" />
			<g:set var="entityName1" value="${message(code: 'home.label', default: 'Home Category')}" />
		...
		<div id="page" style="width:100%;background-color:white;">
	 	...
	 	<script>
			...
			$(function(){
				$('#btn_save').click(function(){
	                   ...
	                   var roleId=selected.id;
	             	   //获取选中的菜单信息
							var homeAuth = $('#home_all').val();
	             	   var url = '${createLink(controller:'homeAuth',action:'save')}';
		                   	$.post(url,{"homeAuth":homeAuth,"roleId":roleId,'datestr':new Date},function(data,textStatus){
		                   		showTopTip(data);
			                },'text');
	             });
			});
			...
			function buildRightGrid(roleId){
		            $('#grid').omGrid({
		                dataSource : "${createLink(controller:'homeAuth',action:'toCategoryList')}?roleId="+roleId,
						limit:0,
		                colModel : [ {header : "${message(code: 'articleCategory.categoryNameC.label', default: 'Category NameC')}", name : 'dicValueNameC', width : 'autoExpand', align : 'left'}, 
		                             {header : "${message(code: 'auth.label', default: 'Auth')}", name : 'homeAuth', align : 'center', width : 100 , renderer : authorityhome}
		                             ],
			...
			function authorityhome(colValue, rowData, rowIndex){
            	if(colValue== 1){
            		$('#home_all').val($('#home_all').val()+rowData.id+'_'); //将已添加权限的栏目code保存到home_all中
