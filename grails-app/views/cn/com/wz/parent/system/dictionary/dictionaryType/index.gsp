...
<%--					<input id="btn_export" type="button"  class="export1" value="${message(code: 'default.button.export.label', default: 'Export')}"/>--%>
				...
				<div style="margin-right:8px;margin-top:8px;">
					<input type="hidden" id="dictionaryItemId">
	 	...
	 	<script>
			...
			$(function() {
                ...
                $('#btn_create').click(function(){
                	   var url = '${createLink(controller:'dictionaryType',action:'create')}';
                ...
                $('#btn_edit').click(function(){
             		   var url = '${createLink(controller:'dictionaryType',action:'edit')}';
             		   var selected = $('#west-panel').omTree('getSelected');
             		   if(selected==null){
                ...
                $('#btn_delete').click(function(){
                	...
                	$.omMessageBox.confirm({
                        ...
                        content:'${message(code: 'dictionaryType.button.delete.confirm.message', default: 'Are you sure?')}',
                        onClose:function(v){
                            if(v){
                            	//需要判断父节点为空的情况
                           		var parent = $('#west-panel').omTree('getParent',selected);
                           		var deleteUrl = '${createLink(controller:'dictionaryType',action:'jsonDelete')}/'+selected.id;
			...
			$(function() {
				...
				//构建控件
			    $('#west-panel').omTree({
			         dataSource : "${createLink(controller:'dictionaryType',action:'jsonListTree')}", // 后台取数的URL
			       	 simpleDataModel:true,
			       	 onSelect: function(node,event) {
			       		  $('#center-panel').css({visibility:'visible'});
			       		  $('#dictionaryTypeId').val(node.id);
		                  $('#content').load("${createLink(controller:'dictionaryType',action:'jsonShow')}/"+node.id);
