...
<head>
    ...
    <g:set var="entityName" value="${message(code: 'resourceRole.label', default: 'Resource Role')}" />
...
<script>
    ...
    $(function() {
        ...
        $('#btn_add').click(function(){
            ...
            //处理没有选中节点的情况
            if(selected==null){
                alert('${message(code: 'default.selection.empty.message', default: 'Please select manager')}');
                return;
            }
            //处理选中的是角色类型的情况
            if(selected.pid==null){
                alert('${message(code: 'default.selection.empty.message', default: 'Please select  one type')}');
                return;
            }
        ...
        $('#btn_delete').click(function(){
            ...
            //处理没有选中节点的情况
            if(selected==null){
                alert('${message(code: 'default.selection.empty.message', default: 'Please select one Result')}');
                return;
            }
            //这里利用Ajax删除数据
            ...
            $.omMessageBox.confirm({
                ...
                onClose:function(v){
                    if(v){
                        var deleteUrl = '${createLink(controller:'resourceRole',action:'jsonDelete')}';
                        var deleteIds = '';
                        $.each(selections,function(i,item){
                            deleteIds += item.id+'_';
                        });
                        $.post(deleteUrl,{"deleteIds":deleteIds,"resourceId":selected.id},function(data,textStatus){
    ...
    $(function() {
        ...
        //构建控件
        $('#west-panel').omTree({
            dataSource : "${createLink(controller:'dictionaryValue',action:'selectDicValueByTypeCode')}?code=dt_sysResMan", // 后台取数的URL
            simpleDataModel:true,
            onSelect: function(node,event) {
                $('#resourceId').val(node.id);//查询时使用
                buildRightGrid(1);
    ...
    function showModelDialog(url){
        var content='<iframe id="iframe_dialog" style="margin-left:-5px;margin-top:-10px;'+
                ...
                'scrolling="no" src="'+'${createLink(controller:'resourceRole',action:'dialog')}'+
                '?resourceId='+$("#resourceId").val()+"&tabId="+window.name+'"></iframe>';
        var title='${message(code:'resourceRole.selectRole.label',default:'Select Role') }';
        //打开弹出框
    ...
    function buildRightGrid(opFlag){
        $('#role_content').omGrid({
            dataSource:'${createLink(controller:'resourceRole',action:'selectRolesByResource')}?opFlag='+opFlag+'&'+$('#form_query').serialize(),
