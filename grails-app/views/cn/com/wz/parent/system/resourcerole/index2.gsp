...
<script>
        ...
        $('#btn_save').click(function(){
            ...
            $.each(selections,function(i,item){
                if(item.status=='value') {
                    resourceIds += item.id+'_';
                }

            });
            var url = '${createLink(controller:'ResourceRole',action:'update')}';
            $.post(url,{"resourceIds":resourceIds,"roleId":selected.id,'datestr':new Date},function(data,textStatus){
    ...
    function buildRightTree(roleId){
        $('#content').omTree({
            dataSource : "${createLink(controller:'ResourceRole',action:'jsonResourceList')}", // 后台取数的URL
            simpleDataModel:true,
            showCheckbox:true,
            onSelect: function(node,event) {
                if(node.isExpand=='0'){
    ...
    function getOldMenu(roleId){
        //给原来的菜单信息进行赋值
        url="${createLink(controller:'ResourceRole',action:'selectOldResourceByRole')}"
