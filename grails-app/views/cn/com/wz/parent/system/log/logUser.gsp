	...
	<body>
		...
		function show(index){
 		   ...
 		   if(data.length==0){
         	   ...
         	   window.location.href = url+'?receiverId='+data.id+'&nodeType=online';
            }
        }
		function buildRightGrid(){
			$('#log_grid').omGrid({
				...
				limit : 0, 
				height : 470,
				colModel:[{header : "用户姓名", name : 'tempName', width : 100, align : 'center', renderer:
						  ...
						  {header : "${message(code: 'user.organization.label', default: 'Organization')}", name : 'organNames', width : 289, align : 'left' ,sort:'clientSide'},
						  {header : "${message(code: 'post.label', default: 'Post')}", name : 'postNames', width : 200, align : 'left' ,sort:'clientSide'},
						  {header : "${message(code: 'onlineUser.startTime.label', default: 'Login Time')}", name : 'startTime', width : 140, align : 'center' ,sort:'clientSide'},
						  {header : "${message(code: 'onlineUser.online.lable', default: 'Online Time')}", name : 'onlineTime', width : 180, align : 'center' ,sort:'clientSide'},
						  {header : "${message(code: 'onlineUser.operat.label', default: 'Send Message')}", name : 'view', width: 100, align:'center', renderer:
                          	 function(colValue, rowData, rowIndex){
                      		 var data = rowData;
                          	 return '<button id="btn_view" class="btn_view" onClick="show('+rowIndex+')">发送消息</button>';
                           	 }}
                          ]
			});
		}
