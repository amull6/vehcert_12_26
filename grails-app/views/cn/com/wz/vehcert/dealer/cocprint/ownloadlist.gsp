		...
		<div id="page" style="width:98%;background-color:white;">
	 	...
	 	<script>
			...
			$(function() {
	                  ...
	                  //绑定查询按钮事件
	                   $('#btn_query').omButton({
		                   		width:80,
		                	   onClick:function(){
				   	                    var url = "${createLink(controller:'cocPrint',action:'jsonOwnLoadList')}";
			...
			function buildRightGrid(){
				$('#zcinfo_grid').omGrid({
					dataSource:'${createLink(controller:'cocPrint',action:'jsonOwnLoadList')}?'+$('#form_query').serialize(),
					...
					colModel:[
                        ...
                        {header : "${message(code: 'zcinfo.opertion.label', default: 'print')}", name : 'id', width: 100, align:'center', renderer:
                                function(value, rowData, rowIndex){
                                    var data = rowData;
                                    return '<a id="btn_view"  class="btn_basic blue_black"  href="javascript:goNewYiZhi('+rowIndex+');">新版证书打印</a>'
                                }},
                        {header : "车辆状态信息", name : 'veh_Clztxx', width : 80, align : 'center'},
                                ...
                                if(value){
                                    ...
                                    $.post(delCocPrUrl,function(data,textStatus){
                                       if(textStatus=="success"){
                                           var url = '${createLink(controller:'cocPrint',action:'zcinfoOfList')}?zcinfoID='+data1.id+'&usertype=1';
                        ...
                        $.omMessageBox.confirm({
                            ...
                            onClose:function(value){
                                if(value){
                                    ...
                                    var url = '${createLink(controller:'cocPrint',action:'cocBookPrint')}?cocPrnID='+data1.id+'&usertype=0&newCoC=1';
                                    window.location.href = url;
                                }
                            }
                        });
