        ...
        <form id="form_query" style="margin:8px;">
            <table style="width:100%;border:0px;">
                <tr>
                    ...
                    <td width="100"><span><g:textField id="serialNum" name="serialNum" maxlength="200" /></span></td>
                    <td width='20'></td>
                    <td width="80"><span><g:message code="Workshop.name.label" default="Name" />：</span></td>
                    <td width="100"><span><g:textField id="name" name="name" maxlength="200" /></span></td>
                    <td width='20'></td>
...
<script>
    ...
    $(function() {
        ...
        $('#btn_query').omButton({
            width:80,
            onClick:function(){
                var url = "${createLink(controller:'WorkshopToSign',action:'jsonList')}";
        ...
        $(".pdf").click(function(){
            ...
            var url="${createLink(controller:'WorkshopToSign',action:'export')}?format=pdf&extension=pdf";
            url=encodeURI(url);
        ...
        $(".excel").click(function(){
            ...
            //当节点没有选中的情况，导出s通讯录中所有的数据
            var url="${createLink(controller:'WorkshopToSign',action:'export')}?format=excel&extension=xlsx&name="+name+"&serialNum="+serialNum;
        ...
        $('#btn_create').click(function(){
            var url = '${createLink(controller:'WorkshopToSign',action:'create')}';
            var selected = $('#workshop_grid').omTree('getSelected');
            window.location.href = url;
        });
        $('#btn_edit').click(function(){
            var url = '${createLink(controller:'WorkshopToSign',action:'edit')}';
        ...
        $('#btn_delete').click(function(){
            ...
            $.omMessageBox.confirm({
                ...
                onClose:function(v){
                    if(v){
                        var deleteUrl = '${createLink(controller:'WorkshopToSign',action:'jsonDelete')}';
                        var deleteIds = '';
        ...
        $('#btn_view').click(function(){
            var url = '${createLink(controller:'WorkshopToSign',action:'show')}';
            var selected = $('#workshop_grid').omGrid('getSelections',true);
            if(selected.length==0){
                alert('${message(code:'default.grid.not.selection',default:'Please select record')}');
            }else if(selected.length>1){
        ...
        $('#btn_imp').click(function(){
            var url='${createLink(controller:'WorkshopToSign',action:'importPage')}';
            showModelDialog(url);
        });
    })
    ...
    function buildRightGrid(){
        $('#workshop_grid').omGrid({
            dataSource:'${createLink(controller:'WorkshopToSign',action:'jsonList')}',
            title:"<g:message code="default.list.label" args="[entityName]"/>",
            singleSelect:false,
            limit : 12,
            height :440,
            colModel:[
                {header : "${message(code: 'Workshop.serialNum.label', default: 'SerialNum')}", name : 'serialNum', width : 100, align : 'center'} ,
                {header : "${message(code: 'Workshop.name.label', default: 'Name')}", name : 'name', width : 150, align : 'center'},
                {header : "${message(code: 'Workshop.kind.label', default: 'Kind')}", name : 'kind', width : 100, align : 'center'},
                {header : "${message(code: 'Workshop.symbol.label', default: 'Symbol')}", name : 'symbol', width : 150, align : 'center'},
                {header : "${message(code: 'Workshop.start.label', default: 'start')}", name : 'startNum', width : 100, align : 'center'},
                {header : "${message(code: 'Workshop.max.label', default: 'max')}", name : 'maxNum', width : 100, align : 'center'},
                {header : "${message(code: 'Workshop.symbolNote.label', default: 'Symbol Note')}", name : 'symbolNote', width :360, align : 'center'}]
        });
    }
