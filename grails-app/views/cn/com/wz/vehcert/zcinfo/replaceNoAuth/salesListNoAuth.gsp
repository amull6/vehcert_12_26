        ...
        %{--<div >--}%
            %{--<div id="export1">--}%
                %{--<fieldset class="buttons" style="margin:0px 8px 8px 4px;">--}%
                    %{--<export:formats formats="['excel']" style="border:0px; margin-top:0px;margin-left:28px;"/>--}%
                %{--</fieldset>--}%
            %{--</div>--}%
            %{--<div id="export2">--}%
                %{--<fieldset class="buttons" style="margin:0px 8px 8px 4px;">--}%
...
<script>
    ...
    $(function() {
        ...
        $('#btn_query').omButton({
            ...
            onClick:function(){
                var id=$('#user').val();
                var url = "${createLink(controller:'ZCInfoNoAuth',action:'jsonListByDealerNoAuth')}?searchType=${searchType}&user.id="+id;
        ...
        $('#btn_query_rq').omButton({
            width:80,
            onClick:function(){
                var url = "${createLink(controller:'ZCInfoNoAuth',action:'jsonListByDealerNoAuth')}?searchType=${searchType}";
                url+='&'+$('#form_query').serialize();
                $('#zcinfore_grid').omGrid('setData',url);
            }
        });
    ...
    function buildRightGridByJC(){
        ...
        $('#zcinfore_grid').omGrid({
            dataSource:'${createLink(controller:'ZCInfoNoAuth',action:'jsonListByDealerNoAuth')}?searchType=${searchType}&'+$('#form_query').serialize(),
            ...
            colModel:[
                ...
                {header : "${message(code: 'zcinfore.salesmanname.label', default: '业务员姓名')}", name : 'salesmanname', width : 120, align : 'center'},
                {header : "${message(code: 'zcinfore.salesmantel.label', default: '业务员电话')}", name : 'salesmantel', width : 120, align : 'center'},
                {header : "${message(code: 'zcinfo.remark.label', default: '申请备注')}", name : 'remark', width : 120, align : 'center'}
            ]
        });
    }
...
//                $("#type1").val("2");
    ...
    function gotoedit(index){
        ...
        var url = '${createLink(controller:'ZCInfoNoAuth',action:'zinfoReplaceEditNoAuth')}?searchType=${searchType}&id='+data.id+'&username='+username;
        window.location.href = url;
    }
    //合格证删除（未审核）
    function deleteSingle(index){
        var data = $("#zcinfore_grid").omGrid("getData").rows[index];
        var url = '${createLink(controller:'ZCInfoNoAuth',action:'deleteSingleNoAuth')}?searchType=${searchType}&id='+data.id;
        $.post(url,{},function(data,textStatus){
            //判断是否已处理完成
            if(data=='success'){
    ...
    function gotoview(index){
        var data = $("#zcinfore_grid").omGrid("getData").rows[index];
        var username =$("#username").val()
        var url = '${createLink(controller:'ZCInfoNoAuth',action:'zcInfoReplaceViewNoAuth')}?id='+data.id+"&searchType=${searchType}"+"&username="+username ;
        window.location.href = url;
    }
