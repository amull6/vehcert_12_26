...
<head>
    ...
    <g:set var="entityName" value="农装生产配置" />
...
<div id="page" style="background-color:white;">
    <div id="center-panel" style="margin-left:4px;border:1px;">
        ...
        <div style="margin-right:8px;margin-top:8px;">
            <div id="harvestConfig_grid" style="border:0px"></div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(function() {
        ...
        $("#btn_save").click(function(){
            var data = $('#harvestConfig_grid').omGrid('getChanges');
            ...
            /*****此处传递data到后台并处理*******/
            var url="${createLink(controller: "harvestConfig",action:"update")}";
            $.post(url,{"updates":updates},function(data,textStatus){
                data=eval("("+data+")");
                if(data.flag=='success'){
                    $('#harvestConfig_grid').omGrid('reload');
    ...
    function buildRightGrid(){
        $('#harvestConfig_grid').omGrid({
            dataSource:"${createLink(controller:'harvestConfig',action:'findConfigDicValue')}",
            ...
            onCancelEdit:function(){
                $("#harvestConfig_grid").omGrid("cancelChanges");
            },
            colModel:[
                {header : "配置项", width : 150, align : 'left',name : 'dicValueNameC', wrap:true, editor:{editable:false}},
                {header : "配置编码", width : 130, align : 'left',name : 'code', wrap:true, editor:{editable:false}},
                {header : "配置参数值", width : 100, align : 'left',name : 'value1',editor:{}},
                {header : "创建人", width : 60, align : 'center',name : 'createdBy', wrap:true, editor:{editable:false}},
