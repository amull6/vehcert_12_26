...
<div id="page" style="background-color:white;">
    <div id="center-panel" style="margin-left:4px;border:1px;">
        ...
        <div style="margin-right:8px;margin-top:8px;">
            <div id="nzConfig_grid" style="border:0px"></div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(function() {
        ...
        $("#btn_save").click(function(){
            var data = $('#nzConfig_grid').omGrid('getChanges');
            ...
            /*****此处传递data到后台并处理*******/
            var url="${createLink(controller: "NZconfig",action:"update")}";
            $.post(url,{"updates":updates},function(data,textStatus){
                data=eval("("+data+")");
                if(data.flag=='success'){
                    $('#nzConfig_grid').omGrid('reload');
    ...
    function buildRightGrid(){
        $('#nzConfig_grid').omGrid({
            dataSource:"${createLink(controller:'NZconfig',action:'findConfigDicValue')}",
            ...
            onCancelEdit:function(){
                $("#nzConfig_grid").omGrid("cancelChanges");
            },
            colModel:[
