...
<script>
    ...
    $(function() {
        ...
        $('#btn_query').omButton({
            width:80,
            onClick:function(){
                var url = "${createLink(controller:'transferToSap',action:'jsonListSearch_harvest')}";
    ...
    function buildRightGrid(){
        $('#transfer_grid').omGrid({
            dataSource:"${createLink(controller:'transferToSap',action:'jsonListSearch_harvest')}?"+$('#form_query').serialize(),
            ...
            colModel:[
                ...
                {header : "新车辆编号", width : 100, align : 'center',name : 'veh_new_clbh'},
    ...
    function transferData(index){
        ...
        $.omMessageBox.prompt({
            ...
            onClose:function(value){
                ...
                var uploadUrl = '${createLink(controller:'transferToSap',action:'transferData_Harvest')}?ZOPIND=1&&opFlag=0';
                var id= data.id;
    ...
    function modify(index){
        ...
        $.omMessageBox.prompt({
            ...
            onClose:function(value){
                ...
                var uploadUrl = '${createLink(controller:'transferToSap',action:'transferData_Harvest')}?ZOPIND=1&&opFlag=1';
                var id= data.id;
                $.post(uploadUrl,{"id":id,"SAP_No":value},function(data,textStatus){
                    if(data.type=='S'){
                        alert(data.msg);
    ...
    function cancleTransfer(index){
        ...
        $.omMessageBox.confirm({
            ...
            onClose:function(v){
                if(v){
                    var uploadUrl = '${createLink(controller:'transferToSap',action:'transferData_Harvest')}?ZOPIND=2&&opFlag=2';
                    var id= data.id;
