...
<div id="edit-user" class="content scaffold-edit" role="main">
    ...
    <g:hasErrors bean="${heavyGas}">
        <ul class="errors" role="alert">
                <g:eachError bean="${heavyGas}" var="error">
                <li <g:if test="${error in org.springframework.validation.FieldError}">data-field-id="${error.field}"</g:if>><g:message error="${error}"/></li>
            </g:eachError>
        </ul>
    </g:hasErrors>
    <g:form method="post" >
        <g:hiddenField name="id" value="${heavyGas?.id}" />
        <g:hiddenField name="version" value="${heavyGas?.version}" />
    ...
    $(".cancel").click(function(){
        ...
        $.omMessageBox.confirm({
            ...
            onClose:function(value){
                if(value){
                    window.location.href="${createLink(controller:'heavyGas',action:'index')}";
                }
            }
        });
    });
