...
<head>
    ...
    <g:set var="entityName" value="双燃料/轻型汽油车环保信息" />
    <title>双燃料/轻型汽油车环保信息</title>
</head>
<body>
            ...
            <table style="width:100%;border:0px;">
                <tr>
                    ...
                    <td style="width: 100px"><span><select name="type" id="type">
                        ...
                        <option value="0">两用燃料车</option>
                        <option value="1">轻型汽油车</option>
                    </select></span></td>
...
<script>
    ...
    $(function() {
        ...
        $('#btn_query').omButton({
            width:80,
            onClick:function(){
                var url = "${createLink(controller:'lightDual',action:'jsonList')}";
                url+='?'+$('#form_query').serialize();
                $('#lightDual_grid').omGrid('setData',url);
            }
        });
        $('#btn_clear').omButton({
            width:80,
        ...
        $(".excel").click(function(){
            ...
            var url="${createLink(controller:'lightDual',action:'exportSearch')}?format=excel&extension=xlsx&en_clxh="+en_clxh+"&type="+en_clxh+"&en_fdjxh="+en_fdjxh+"&en_xxgkbh="+en_xxgkbh;
            url=encodeURI(url);
            window.location.href=url;
        });
        $('#btn_edit').click(function(){
            var url = '${createLink(controller:'lightDual',action:'edit')}';
            var selected = $('#lightDual_grid').omGrid('getSelections',true);
        ...
        $('#btn_delete').click(function(){
            //这里利用Ajax删除数据
            var selections = $('#lightDual_grid').omGrid('getSelections',true);
            ...
            $.omMessageBox.confirm({
                ...
                onClose:function(v){
                    if(v){
                        var deleteUrl = '${createLink(controller:'lightDual',action:'jsonDelete')}';
                        var deleteIds = '';
                        $.each(selections,function(i,item){
                            deleteIds += item.id+'_';
                        });
                        $.post(deleteUrl,{"deleteIds":deleteIds},function(data,textStatus){
                            var selections = $('#lightDual_grid').omGrid('reload');
                            var content = data;
                            /**
        ...
        $('#btn_view').click(function(){
            var url = '${createLink(controller:'lightDual',action:'show')}';
            var selected = $('#lightDual_grid').omGrid('getSelections',true);
            if(selected.length==0){
                alert('${message(code:'default.grid.not.selection',default:'Please select record')}');
            }else if(selected.length>1){
        ...
        $('#btn_imp').click(function(){
            var url='${createLink(controller:'lightDual',action:'importPage')}';
            showModelDialog(url);
        });
    })
    ...
    //获取组合结果
    function getResult(){
        var url='${createLink(controller:'lightDual',action:'getCompResult')}';
