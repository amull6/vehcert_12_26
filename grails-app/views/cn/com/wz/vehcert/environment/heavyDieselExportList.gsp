...
<head>
    ...
    <g:set var="entityName" value="重型柴油车/城市车辆环保清单信息" />
    <title>重型柴油车/城市车辆环保清单信息</title>
</head>
<body>
<div id="page" style="background-color:white;">
    <div id="center-panel" style="margin-left:4px;border:1px;">
        ...
        <fieldset class="buttons" style="margin:10px 10px 10px 10px;">
            <export:formats formats="['excel']"  controller="oil" action="export" style="border:0px;margin-left:22px;margin-top:0px;"/>
        </fieldset>
        <div style="margin-right:8px;margin-top:8px;">
            <div id="heavyDieselInfo_grid" style="border:0px"></div>
        </div>
    </div>
</div>
<script>
    ...
    $(function() {
        ...
        $('#btn_query').omButton({
            width:80,
            onClick:function(){
                var url = "${createLink(controller:'heavyDiesel',action:'jsonListInfo')}";
                url+='?'+$('#form_query').serialize();
                $('#heavyDieselInfo_grid').omGrid('setData',url);
            }
        });
        $('#btn_clear').omButton({
            width:80,
        ...
        $(".excel").click(function(){
            ...
            var url="${createLink(controller:'heavyDiesel',action:'heavyDieselInfoExportSearch')}?format=excel&extension=xlsx&en_clxh="+en_clxh+"&type="+en_clxh+"&en_fdjxh="+en_fdjxh+"&en_xxgkbh="+en_xxgkbh;
            url=encodeURI(url);
    ...
    function buildRightGrid(){
        $('#heavyDieselInfo_grid').omGrid({
            dataSource:'${createLink(controller:'heavyDiesel',action:'jsonListInfo')}',
            title:"<g:message code="default.list.label" args="[entityName]"/>",
            singleSelect:false,
            limit : 10,
            height :390,
            colModel:[
                ...
                {header : "VIN", name : 'en_clsbdh', width : 250, align : 'center'},
    ...
    function backFunc(result){
        $('#heavyDieselInfo_grid').omGrid('reload');
    }
