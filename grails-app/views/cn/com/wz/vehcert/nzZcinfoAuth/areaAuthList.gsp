...
<html>
	<head>
		...
		<g:set var="entityName" value="区域经理审核" />
	 	...
	 	<script>
			...
			$(function() {
	                  ...
	                  //绑定查询按钮事件
	                   $('#btn_query').omButton({
		                	   ...
		                	   onClick:function(){
                                   var id=$('#user').val();
				   	                    var url = "${createLink(controller:'NZzcinfoReplaceAuth',action:'jsonListByArea')}";
			...
			function buildRightGrid(){
				$('#zcinfore_grid').omGrid({
					dataSource:'${createLink(controller:'NZzcinfoReplaceAuth',action:'jsonListByArea')}?area_status=0&searchType='+$('#searchType').val(),
            ...
            function gotoview(index){
                ...
                var url = '${createLink(controller:'NZzcinfoReplaceAuth',action:'areaAuditView')}?id='+data.id+'&searchType='+$('#searchType').val();
                 window.location.href = url;
