...
<tr class="om-grid-row evenRow">
    <td align="left" class="indexCol" colspan="17"><div align="left" class="indexCol" >区域经理审核备注：${nzZcinfoReAuInstance?.area_Remark}
    </div></td>
</tr>
<tr class="om-grid-row oddRow">
...
<tr class="om-grid-row oddRow">
    <td align="left" class="indexCol" colspan="4"><div align="left" class="innerCol "col style="width:100px" >
        1.车辆编号</div></td>
    ...
    <td align="left" class="indexCol" colspan="4"><div align="left" class="innerCol" <g:if test="${nzZcinfoReAuInstance?.change_Field=='veh_Clbh'}"> style="color: red" </g:if> >
        1.车辆编号</div></td>
    <td align="left" abbr="id" class="col0" colspan="4">
        <g:if test="${nzZcinfoReAuInstance?.change_Field=='veh_Clbh'}">
            <div align="left" class="innerCol " style="color: red">
                <input type="text" id="veh_Clbh_R" name='veh_Clbh_R' value="">
            </div>
        </g:if>
        <g:else>
            ${nzZcinfoReAuInstance?.veh_Clbh}
        </g:else>
       </td>
    </tr>
...
<tr class="om-grid-row evenRow">
    ...
    <td align="left" abbr="id" class="col0" colspan="4">
        <g:if test="${nzZcinfoReAuInstance?.change_Field=='veh_Clbh'}">
            <div align="left" class="innerCol " style="color: red">
                <input type="text" id="veh_Dph_R" name='veh_Dph_R' value="">
            </div>
        </g:if>
        <g:else>
            ${nzZcinfoReAuInstance?.veh_Dph}
        </g:else>
...
<tr class="om-grid-row evenRow">
    ...
    <g:textArea name="auth_Remark" id="auth_Remark" cols="40" rows="2" maxlength="500" style="width:600px;height:40px;" value="${nzZcinfoReAuInstance?.auth_Remark}"/>
    </div></td>
</tr>
<tr class="om-grid-row evenRow">
    <td  abbr="id" class="col0" colspan="17"><div align="right" class="innerCol ">
        <form id="form" method="post">
            ...
            <input type="hidden" value="${nzZcinfoReAuInstance?.change_Field}" id="change_Field">
            <g:hiddenField name="msg" id='msg' value="${params?.msg}"/>
            &nbsp&nbsp&nbsp
            <g:if test="${nzZcinfoReAuInstance?.auth_status==0}">
...
<script>
    $(function() {
        ...
        $("#btn_audit_pass").click(function(){
            var auth_Remark=$("#auth_Remark").val()
            var veh_Clbh_R=$('#veh_Clbh_R').val()
            var veh_Dph_R=$('#veh_Dph_R').val()
            var change_Field=$('#change_Field').val()

            if(change_Field=='veh_Clbh'){
                if(veh_Clbh_R==''||veh_Clbh_R==null){
                        alert('请填写更换后车辆编号');
                        return
                }
                if(veh_Dph_R==''||veh_Dph_R==null){
                    alert('请填写更换后底盘号');
                    return
                }
                if(veh_Dph_R!=veh_Clbh_R){
                    alert('车辆编号要与底盘号一致');
                    return
                }
                if(auth_Remark==''||auth_Remark==null){
                    alert('请填写审核备注');
                    return
                }
                var url = '${createLink(controller:'NZzcinfoReplaceAuth',action:'companyAudit')}?id='+$("#nzZcinfoReplaceId").val()+"&auth_Remark="+encodeURI($("#auth_Remark").val())+'&change_Field='+change_Field+'&veh_Clbh_R='+veh_Clbh_R+'&veh_Dph_R='+veh_Dph_R+"&pass=Y";
            }else{
                if(auth_Remark==''||auth_Remark==null){
                    alert('请填写审核备注');
                    return
                }
                var url = '${createLink(controller:'NZzcinfoReplaceAuth',action:'companyAudit')}?id='+$("#nzZcinfoReplaceId").val()+"&auth_Remark="+encodeURI($("#auth_Remark").val())+"&pass=Y"+'&change_Field='+change_Field;
            }
        ...
        $("#btn_audit_noPass").click(function(){
            ...
            var url = '${createLink(controller:'NZzcinfoReplaceAuth',action:'companyAudit')}?id='+$("#nzZcinfoReplaceId").val()+"&auth_Remark="+encodeURI($("#auth_Remark").val())+"&pass=N";
            window.location.href = url;
        });
        $("#btn_return").click(function(){
            var url = '${createLink(controller:'NZzcinfoReplaceAuth',action:'authList')}?searchType=1';
            window.location.href = url;
        });
    })
