...
<head>
    ...
    <g:set var="entityName" value="农装合格证更换打印" />
        ...
        <form id="form_query" style="margin:8px;height:40px;">
            <table style="width:100%;border:0px;">
                <tr>
                    ...
                    <td width="60px"  align="right"><span>打印状态：</span></td>
                    <td><span>
...
<script>
    ...
    $(function() {
        ...
        $('#btn_query').omButton({
            ...
            onClick:function(){
                var id=$('#user').val();
                var url = "${createLink(controller:'NZzcinfoReplaceAuth',action:'jsonListForChangPrint')}?+&auth_status=1";
    ...
    function buildRightGrid(){
        $('#zcinfore_grid').omGrid({
            dataSource:'${createLink(controller:'NZzcinfoReplaceAuth',action:'jsonListForChangPrint')}?searchType='+$('#searchType').val()+'&isPrint=0&auth_status=1',
            ...
            colModel:[
                {header : "${message(code: 'zcinfo.opertion.label', default: '操作')}", name : 'id', width: 40, align:'center', renderer:
                        function(value, rowData, rowIndex){
                            var data = rowData;
                            return '<a id="btn_view" class="btn_basic blue_black"  href="javascript:goToPrint('+rowIndex+')">打印</a>'
                        }},

                {header : "经销商", name : 'createName', width : 80, align : 'left'},
                ...
                {header : "旧合格证编号", name : 'veh_Hgzbh', width : 100, align : 'center'},
                ...
                {header : "新合格证编号", name : 'veh_Hgzbh_R', width : 100, align : 'center'},
//                {header : "业务员电话", name : 'salesManPhone', width : 75, align : 'center'},
                {header : "打印状态", name : 'isPrint', width : 60, align : 'center',
    ...
    function goToPrint(index){
        ...
        var url = '${createLink(controller:'NZzcinfoReplaceAuth',action:'goToPrint')}?id='+data.id;
        window.location.href = url;
