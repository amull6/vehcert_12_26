...
//    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]
     ...
     */
    def list() {
        render (view: "/cn/com/wz/vehcert/environment/heavyDiesel/list",model: params)
    }
    ...
    def jsonList(){
        ...
        def result=heavyDieseService.selectByParams(params)
     ...
     */
    def importPage(){
        render(view:'/cn/com/wz/parent/common/import',model:[redirectUrl:"${createLink(controller: 'heavyDiesel',action: 'imp')}",tabId:params.tabId])
    }
...
//                    count = lst.size()
                ...
                checkResult=heavyDieseService.checkGkxxNo(lst)   //不再校验公告唯一号的唯一性
...
//                turnOffCarStorage=checkResult.turnOffCarStorage
                ...
                if(emptyCount==0){ // 如果没有信息公开编号为0的数据库
                    def m=heavyDieseService.newSqlSaveList(paramsMap,request,grailsAttributes)
                    timeCount=m.timeCount
                    count = lst.size()
                }else{
    ...
    def exportSearch(){
        ...
        try{
            if(params?.format && params.format != "html"){
                def encodedFilename = URLEncoder.encode(("重型柴油车城市车辆环保信息导出"), "UTF-8")
                def filename = ""
                ...
                Map labels =  ["en_xxgkbh":"信息公开号",
                                ...
                                "en_jcbz3":"HJ 689-2014",
                                "en_jcbz4":"HJ 437-2008",
                                "en_jcbz5":"HJ 438-2008 ",
                                ...
                                "en_pqhclxtxh":"排气后处理系统型号",
                                 "en_tc":"SCR封装/载体/涂层生产厂",
                                 "en_tc1":"DOC封装/载体/涂层生产厂",
                                "en_pqhclxtxs":"排气后处理系统形式",
                                "en_klqxh":"空滤器型号",
                                "en_jqxsqxh":"进气消声器型号",
                                "en_pqxsqxh":"排气消声器型号",
                                "type":"清单型号 ",
                ...
                def map=heavyDieseService.selectByParams(params)
                ...
                map.clear()
                excelOp.preWriteExcel(out,null,m,["重型柴油车城市车辆环保信息导出"],content)
     ...
     */
    def show(){
        def heavyDiesel=HeavyDiesel.get(params.id)
        render (view:'/cn/com/wz/vehcert/environment/heavyDiesel/show',model: [heavyDiesel:heavyDiesel])
    }
    ...
    def edit() {
        ...
        render (view:'/cn/com/wz/vehcert/environment/heavyDiesel/edit',model: [heavyDiesel: heavyDiesel])
    }
    /**
     * @DEscription 修改数据
     * @return
     */
    def update() {
        def heavyDieselInstance = HeavyDiesel.get(params.id)
        if (params.version&&params.version<heavyDieselInstance.version){
            flash.message='记录已过期！'
            return render (view:'/cn/com/wz/vehcert/environment/heavyDiesel/edit',model: [heavyDiesel:heavyDieselInstance])
        }
        String curTime=DateUtil.getCurrentTime()
        heavyDieselInstance.properties=params
        heavyDieselInstance.updater_id= User.get(getCurrentUser()?.id)?.userName
        heavyDieselInstance.update_time=curTime

        if (!heavyDieselInstance.save(flush: true)){
            return render (view:'/cn/com/wz/vehcert/environment/heavyDiesel/edit',model: [heavyDiesel:heavyDieselInstance])
        }else{
            flash.message="更新成功！"
            redirect(action: "show", id: heavyDieselInstance.id)
        }
    }
    ...
    def jsonDelete(){
        ...
        idsArray.each {
            def heavyDiesel = HeavyDiesel.get(it)
            if (!heavyDiesel) {
                return
            }
            try {
                heavyDiesel.delete(flush: true)
