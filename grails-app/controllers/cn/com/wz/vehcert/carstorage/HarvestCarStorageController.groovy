 ...
 */
class HarvestCarStorageController extends BaseController {
    def dmsSynService
    def sqlToolService
    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]
    def harvestCarStorageService
        def exportService
        def dataSource
        def sqlService
        def index() {
            render (view:'/cn/com/wz/vehcert/carstorage/harvestCarstorage/harvestCarStorageList',model: params)
        }
        /**
        ...
        def result=harvestCarStorageService.selectHarvestCarStorageByParams(params)
     ...
     */
    def importPage(){
        render(view:'/cn/com/wz/parent/common/import',model:[redirectUrl:"${createLink(controller: 'harvestCarStorage',action: 'imp')}",tabId:params.tabId])
    }
    /**
     * @Description 公告信息导入
     * @param filePath
        ...
        try{
            ...
            if(totalCount>0){
              //对list里面的的公告唯一码进行校验，如果没有查到相同公告唯一码的公告，就做公告导入处理
                ...
                checkResult=harvestCarStorageService.checkCarStorageNo(lst)   //不再校验公告唯一号的唯一性
                ...
                if(emptyCount==0&&turnOffCarStorage.size()==0){ // 导入时不再校验公告唯一号是不是唯一的，只校验公告是不是空的、公告是不是已经停用
                    def m=harvestCarStorageService.newSqlSaveList(paramsMap,request,grailsAttributes)
     ...
     */
    def toCreate(){
        render (view:'/cn/com/wz/vehcert/carstorage/harvestCarstorage/harvestCreate')
    }
     ...
     */
    def save(){
        def harvestCarStorage=new HarvestCarStorage(params)
        harvestCarStorage.createrId=getCurrentUser()?.id
        harvestCarStorage.createTime=DateUtil.getCurrentTime()
        if (!harvestCarStorage.save(flash:true)){
            render (view:'/cn/com/wz/vehcert/carstorage/harvestCarstorage/harvestCreate',model: [harvestCarStorage:harvestCarStorage])
            return
        }else{
...
//            servletContext.setAttribute("dic_maxCarStorageNo",maxCarStorageNoIns)
            flash.message="保存成功！"
            redirect(action: "show", id: harvestCarStorage.id)
        }
    }
     ...
     */
    def toUpdate(){
        def harvestCarStorage=HarvestCarStorage.get(params.id)
        render (view:'/cn/com/wz/vehcert/carstorage/harvestCarstorage/harvestEdit',model: [harvestCarStorage: harvestCarStorage])
    }

    def update(){
        def harvestCarStorage=HarvestCarStorage.get(params.id)
        if (params.version&&params.version<harvestCarStorage.version){
            flash.message='记录已过期！'
            return render (view:'/cn/com/wz/vehcert/carstorage/harvestCarstorage/harvestEdit',model: [harvestCarStorage:harvestCarStorage])
        }
        String curTime=DateUtil.getCurrentTime()
        harvestCarStorage.properties=params
        harvestCarStorage.updaterId=getCurrentUser()?.id
        harvestCarStorage.updateTime=curTime

        if (!harvestCarStorage.save(flush: true)){
            return render (view:'/cn/com/wz/vehcert/carstorage/harvestCarstorage/harvestEdit',model: [harvestCarStorage:harvestCarStorage])
        }else{
            HarvestTemp t=new HarvestTemp()
            t.wzh_carstorageID=harvestCarStorage.id
            t.wzh_type=0
...
//            t.car_storage_no=carStorage.car_storage_no
    ...
    def show(){
        def harvestCarStorage=HarvestCarStorage.get(params.id)
        render (view:'/cn/com/wz/vehcert/carstorage/harvestCarstorage/harvestShow',model: [harvestCarStorage:harvestCarStorage])
    }
    ...
    def jsonDelete(){
        ...
        def count=0
        ids?.each{
            def harvestCarStorage= HarvestCarStorage.get(it)
            def car_storage_no=harvestCarStorage.car_storage_no
           if(harvestCarStorage?.delete(flush: true)){
               count+=1
               message+="【${harvestCarStorage.veh_Clxh}】删除失败！_"
           }else{
               HarvestTemp t=new HarvestTemp()
    ...
    def disableCarStorage(){
        ...
        ids?.each{   //先查看勾选的记录中有没有已经停用的公告
            def harvestCarStorage = HarvestCarStorage.get(it)
            if(harvestCarStorage.turnOff=='1'){
                turnOff=turnOff+1
            }
        }
       if(turnOff>0){     //如果勾选记录中有公告处于停用公告，给予不能重复停用提示
           ...
           ids?.each{
               ...
               def car_storage_no=harvestCarStorage.car_storage_no
               //对停用公告做逻辑删除
               harvestCarStorage.setTurnOff('1') //停用标识   0：启用 1：停用
               harvestCarStorage.setTurnOffTime(DateUtil.getCurrentTime())
               if(!harvestCarStorage?.save(flush: true)){
                   count+=1
                   message+="【${harvestCarStorage.veh_Clxh}】停用失败！_"
               }else{
                   HarvestTemp t=new HarvestTemp()
                   t.wzh_carstorageID=it
                   t.wzh_type=0
    ...
    def jsonDeleteSearch(){
        ...
        def message=""
        HarvestCarStorage.withTransaction {trans->
            try{
                ////查询经销商所拥有的合格证信息
                StringBuffer sql_sbfS = new StringBuffer("SELECT ID,car_storage_no FROM wzh_harvestcar_Storage ");
                if(params.veh_Jxxlb){
                     if(params.veh_Clxh){
                ...
                if (listS){
                    StringBuffer sql_sbf = new StringBuffer("DELETE FROM wzh_harvest_carStorage ");
                    if(params.veh_Jxxlb){
                        if(params.veh_Clxh){
    ...
    def disableSearchCarStorage(){
        ...
        HarvestCarStorage.withTransaction {trans->
            try{
                ////查询经销商所拥有的合格证信息
                def need=false
                StringBuffer sql_sbfS = new StringBuffer("SELECT ID,car_storage_no,turn_Off FROM wzh_harvest_carStorage ");
                if(params.veh_Jxxlb){
...
//                println(turnOffCount)
                if(turnOffCount>0){ //如果有已停用的公告，给予不能重复停用提示
                    ...
                    if (listS){
                        StringBuffer sql_sbf = new StringBuffer("update  wzh_harvest_carStorage set turn_Off='1' , turn_off_time='"+turn_off_time+"'");
                        def jion=false
                        if(params.veh_Jxxlb){
...
//                        println(sql_sbf)
    ...
    def exportSearch(){
        ...
        if(params?.format && params.format != "html"){
            ...
            Map labels =  ["veh_Clxh":"车辆型号","veh_Fdjxh":"发动机型号","veh_Jxdl":"机械大类","veh_Rllx":"燃料类型", "veh_Jxxlb":"机械小类别","veh_Zycs":"主要参数","veh_Pfjd":"排放阶段",
                            "car_storage_type":"公告类型（0表示玉米收；1、稻麦机）","car_storage_no":"公告唯一号","turnOff":"公告启用标识（0:启用；1：停用）","turnOffTime":"公告停用时间"]
            def upperCase = { domain, value ->
                return value.toUpperCase()
            }
            def out=response.outputStream
            def map=harvestCarStorageService.selectHarvestCarStorageByParams(params)
