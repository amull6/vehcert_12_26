    ...
    def checkCarStorageNo(lst){
        ...
        lst.each {
            ...
            if(it.en_xxgkbh=='NULL'||it.en_xxgkbh=='null'||!it.en_xxgkbh){
                ...
                def findHeavy_Gas=HeavyGas.findAllByEn_xxgkbh(it.en_xxgkbh?.toString()?.trim())
 ...
 * @CreateTime 2016-12-21 by Qj
    ...
    def selectByParams(params){
        ...
        def lst=HeavyGas.createCriteria().list([max: params.max,offset: params.offset],cel)
        def list=[]
        lst?.each {
            def m=[:]
            ...
            if(existsNoList.size()>0){
                existsNoList?.each{ existsCarStorageNo->
                    ...
                    if(count<999){             //in 中的最大表达式数为 1000
                        if (count==0){
                            sb=new StringBuffer("delete from wzh_heavy_gas  where en_xxgkbh in (")
                        ...
                        sb=new StringBuffer("delete from wzh_heavy_gas  where en_xxgkbh in ('${existsCarStorageNo?.toString()?.trim()}'")
                        count=count-999
                    }
                    count+=1
                }
...
//                    println(timeCount)
            ...
            if (lst?.size()>0){
                ...
                lst?.each{r->
                    ...
                    StringBuffer sbf = new StringBuffer("insert into wzh_heavy_gas ");
