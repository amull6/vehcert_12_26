...
class LightDieselService {
    ...
    def saveList(lst,user,time,request,grailsAttributes){
        ...
        lst?.each{ pct->
            ...
            def lightDiesel=new LightDiesel()
            pct.each {
//                  def  car_storage_no=Integer.parseInt(it.value)
                lightDiesel."${it.key}"=  it.value?(it.value.toString().toUpperCase()=="NULL"?'':it.value):''  //将NULL的内容保存为空
            }
            lightDiesel.creater_id=user?.userName
            lightDiesel.create_time=time
            curCount+=1
            if(lightDiesel.save()){
                count+=1
                println(count)
                ...
                def num=1
                //错误信息处理
                lightDiesel.errors.allErrors?.each{
    ...
    def selectByParams(params){
        def cel={
            ...
            if(params.en_type){
                if(params.en_type=='LightDiesel4'){
                    eq("type","0")
                }else if(params.en_type=='LightDiesel'){
                    eq("type","1")
                }
        ...
        def lst=LightDiesel.createCriteria().list([max: params.max,offset: params.offset],cel)
        def list=[]
        lst?.each {
            def m=[:]
    ...
    def checkCarStorageNo(lst){
        ...
        lst.each {
            ...
            if(it.en_xxgkbh=='NULL'||it.en_xxgkbh=='null'||!it.en_xxgkbh){
                ...
                def findLight_diesel=LightDiesel.findAllByEn_xxgkbh(it.en_xxgkbh?.toString()?.trim()) //因为信息公开编号唯一主键，所以查出来的数据只有一条
                if(findLight_diesel&&findLight_diesel.size()>0){
            ...
            if(existsNoList.size()>0){
                existsNoList?.each{ existsCarStorageNo->
                    ...
                    if(count<999){             //in 中的最大表达式数为 1000
                        if (count==0){
                            sb=new StringBuffer("delete from wzh_light_diesel  where en_xxgkbh in (")
                        ...
                        sb=new StringBuffer("delete from wzh_light_diesel  where en_xxgkbh in ('${existsCarStorageNo?.toString()?.trim()}'")
                        count=count-999
                    }
                    count+=1
                }
...
//                    println(timeCount)
            ...
            if (lst?.size()>0){
                ...
                lst?.each{r->
                    ...
                    StringBuffer sbf = new StringBuffer("insert into wzh_light_diesel ");
