...
class LightDualService {
    ...
    def selectByParams(params){
        def cel={
            ...
            if(params.en_type){
                if(params.en_type=='LightDual'){
                    eq("type","0")
                }else if(params.en_type=='LightPetrol'){
                    eq("type","1")
                }
        ...
        def lst=LightDual.createCriteria().list([max: params.max,offset: params.offset],cel)
        def list=[]
        lst?.each {
            def m=[:]
    ...
    def checkGkxxNo(lst){
        ...
        lst.each {
            ...
            if(it.en_xxgkbh=='NULL'||it.en_xxgkbh=='null'||!it.en_xxgkbh){
                ...
                def findHeavy_diesel=LightDual.findAllByEn_xxgkbh(it.en_xxgkbh?.toString()?.trim()) //因为信息公开编号唯一主键，所以查出来的数据只有一条
            ...
            if(existsNoList.size()>0){
                existsNoList?.each{ existsCarStorageNo->
                    ...
                    if(count<999){             //in 中的最大表达式数为 1000
                        if (count==0){
                            sb=new StringBuffer("delete from WZH_LIGHT_DUAL  where en_xxgkbh in (")
                        ...
                        sb=new StringBuffer("delete from WZH_LIGHT_DUAL  where en_xxgkbh in ('${existsCarStorageNo?.toString()?.trim()}'")
                        count=count-999
                    }
                    count+=1
                }
...
//                    println(timeCount)
            ...
            if (lst?.size()>0){
                ...
                lst?.each{r->
                    ...
                    StringBuffer sbf = new StringBuffer("insert into WZH_LIGHT_DUAL ");
