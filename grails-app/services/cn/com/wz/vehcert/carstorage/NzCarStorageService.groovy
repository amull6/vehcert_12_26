...
class NzCarStorageService {
     ...
     */
    def selectNzCarStorageByParams(params){
        def cel={
            if(params.veh_Clxh){
                  ...
                  if(existsCarStorage){//将Excle中已经存在的公告唯一号的公告先保存到TEMP表，但是不保存公告唯一号，在将这条公告删除
                      ...
                      if(t.save(flush: true)){ //如果TEMP表中更新的记录保存成功，那么将这条已经存在的公告唯一号删除
                              ...
                              if(count<999){             //in 中的最大表达式数为 1000
                                  if (count==0){
                                      sb=new StringBuffer("delete from wzh_NZcarStorage  where car_storage_no in (")
                                  ...
                                  sb=new StringBuffer("delete from wzh_NZcarStorage  where car_storage_no in ('${existsCarStorageNo?.toString()?.trim()}'")
                                  count=count-999
                              }
                              count+=1
                      }
...
//                    println(timeCount)
            ...
            if (lst?.size()>0){
                ...
                lst?.each{r->
                    ...
                    StringBuffer sbf = new StringBuffer("insert into wzh_NZcarStorage ");
    ...
    def checkCarStorageNo(lst){
        ...
        lst.each {
            ...
            if(it.car_storage_no=='NULL'||it.car_storage_no=='null'||!it.car_storage_no){
                ...
                def findCarStorage=NzCarStorage.findAllByCar_storage_no(it.car_storage_no?.toString()?.trim()) //因为公告唯一号是唯一主键，所以查出来的数据只有一条
