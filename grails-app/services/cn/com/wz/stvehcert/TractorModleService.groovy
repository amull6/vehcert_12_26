...
import cn.com.wz.vehcert.carstorage.NzTemp

/**
 * @Description 公告基础信息Service
 * @Create 2013-07-26 huxx
 */
class TractorModleService {
    def sqlToolService
    def logService
     ...
     */
    def selectStCarStorageByParams(params){
        def cel={
            if(params.modleName){
...
//        def lst1=CarStorage.createCriteria().list(cel)
        ...
        lst?.each {
            ...
            m.id="${it.id}"
            def u=User.get(it.createid)
            m.createrName=u?.userDetail?.realName
            def uU=User.get(it.lastUpdateId)
            if (uU){
        ...
        try{
            ...
            if(existsCarStorageNoList.size()>0){
                existsCarStorageNoList?.each{ existsCarStorageNo->
                  ...
                  def  existsCarStorage=TractorModle.findByCar_storage_no(existsCarStorageNo)   //在导入service方法被调用前，已经对导入的Excle公告进行校验过，在这里再次检验公告为启用状态
                  if(existsCarStorage){//将Excle中已经存在的公告唯一号的公告先保存到TEMP表，但是不保存公告唯一号，在将这条公告删除
                      ...
                      if(t.save(flush: true)){ //如果TEMP表中更新的记录保存成功，那么将这条已经存在的公告唯一号删除
                              ...
                              if(count<999){             //in 中的最大表达式数为 1000
                                  if (count==0){
                                      sb=new StringBuffer("delete from WZH_TRACTOR_MODLE  where car_storage_no in (")
                                  ...
                                  sb=new StringBuffer("delete from WZH_TRACTOR_MODLE  where car_storage_no in ('${existsCarStorageNo?.toString()?.trim()}'")
                                  count=count-999
                              }
                              count+=1
                      }
...
//                    println(timeCount)
            ...
            if (lst?.size()>0){
                ...
                lst?.each{r->
                    ...
                    StringBuffer sbf = new StringBuffer("insert into WZH_TRACTOR_MODLE ");
                    UUID uuid = UUID.randomUUID()
                    def id=uuid.toString().replace('-','')
                    StringBuffer title=new StringBuffer(" (ID,CREATEID,CREATE_TIME")
                    StringBuffer values=new StringBuffer(" values('${id}','${user.userName}','${time}'")
                    r.entrySet().each{v->
                        def noNeed=["modleName","veh_VinFourBit"]
                        if (!noNeed.contains(v.key) ){
                            title.append(",${v.key}")
                            values.append(",'${ v.value?(v.value.toString().toUpperCase()=="NULL"?'':v.value):''}'")
                        }else if(v.key=='modleName'){
                            title.append(",modle_Name")
                            values.append(",'${ v.value?(v.value.toString().toUpperCase()=="NULL"?'':v.value):''}'")
                        }
                    }
                    sbf.append(title+")")
    ...
    def checkCarStorageNo(lst){
        ...
        lst.each {
            ...
            if(it.car_storage_no=='NULL'||it.car_storage_no=='null'||!it.car_storage_no){
                ...
                def findCarStorage=TractorModle.findAllByCar_storage_no(it.car_storage_no?.toString()?.trim()) //因为公告唯一号是唯一主键，所以查出来的数据只有一条
